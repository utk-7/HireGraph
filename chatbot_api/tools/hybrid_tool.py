...
from chatbot_api.tools.cypher_tool import NEO4J_URI, NEO4J_USER, NEO4J_PASSWORD
...
def scoped_vector_search(ids: list[str], question: str, top_k: int = 5) -> list[dict]:
    ...
    try:
        with driver.session() as session:
            result = session.run(cypher_query, id_list=ids, query_vector=query_vector, top_k=top_k)
...
def hybrid_rag_tool(cypher_query: str, semantic_search_query: str) -> str:
    ...
    try:
        ...
        reviews_context = "\n---\n".join([f"Type: {r['type']}\nText: {r['text']}\n(Relevance Score: {r['score']:.2f})" for r in reviews])
