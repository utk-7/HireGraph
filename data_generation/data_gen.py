...
def generate_review_text(sentiment, review_type):
    if review_type == "interview_experience":
        if sentiment == "positive":
            openings = ["I had a great time interviewing here.", "The interview process was very smooth.", "Really enjoyed meeting the team."]
            details = ["The recruiters were responsive.", "Technical questions were fair and relevant.", "The panel was very welcoming."]
            closings = ["Would highly recommend.", "Looking forward to hearing back.", "A very professional experience overall."]
        elif sentiment == "negative":
            openings = ["The interview process was frustrating.", "I had a terrible experience interviewing.", "Very disjointed interview process."]
            details = ["The interviewer showed up late.", "Technical questions were completely unrelated to the job.", "Recruiter ghosted me for weeks."]
            closings = ["Would not recommend applying.", "I withdrew my application.", "A huge waste of time."]
        else:
            openings = ["The interview was okay.", "Standard interview process.", "Nothing special about the interview."]
            details = ["Questions were standard Leetcode.", "Met with a few team members.", "Process took a few weeks."]
            closings = ["Overall an average experience.", "We will see what happens.", "Standard corporate process."]
    else:
        if sentiment == "positive":
            openings = ["I love working here.", "This is a great company.", "Fantastic culture and people."]
            details = ["Management really cares about you.", "Great work-life balance.", "Lots of opportunities for growth."]
            closings = ["I plan to stay here a long time.", "Highly recommend it to anyone.", "Best job I've had."]
        elif sentiment == "negative":
            openings = ["Working here has been difficult.", "I do not recommend this company.", "Toxic environment."]
            details = ["Management is completely out of touch.", "Zero work-life balance.", "No clear path for promotion."]
            closings = ["I am actively looking for a new job.", "Avoid at all costs.", "Turnover is very high for a reason."]
        else:
            openings = ["It's an okay place to work.", "Average corporate job.", "Pays the bills."]
            details = ["Some teams are better than others.", "Benefits are standard.", "Work can be monotonous."]
            closings = ["Not bad, but not great.", "I might stay a year or two.", "It's just a job."]
            
    return f"{random.choice(openings)} {random.choice(details)} {random.choice(closings)}"
...
class DataGenerator:
    def __init__(self, seed: int = 42, scale_factor: int = 1):
        ...
        self.nodes = {
            "Company": [], "Department": [], "JobPosting": [], "Candidate": [],
            "Application": [], "Interview": [], "Recruiter": [], "Interviewer": [],
            "Offer": [], "Employee": [], "Review": []
        }
        self.relationships = []
    ...
    def add_rel(self, source_type, source_id, rel_type, target_type, target_id, props=None):
        rel = {
    ...
    def generate_all(self):
        ...
        departments = ["Engineering", "Sales", "Marketing", "HR", "Product", "Design", "Finance"]
        ...
        for c_idx in range(num_companies):
            ...
            self.nodes["Company"].append({
                ...
                "industry": random.choice(["Tech", "Healthcare", "Finance", "Retail", "Energy"]),
            ...
            for dept_name in deps_for_company:
                ...
                self.nodes["Department"].append({
                    "id": dept_id,
                    "name": dept_name
                })
                self.add_rel("Company", company_id, "HAS_DEPARTMENT", "Department", dept_id)
                ...
                for _ in range(num_interviewers):
                    ...
                    self.add_rel("Interviewer", int_id, "WORKS_IN", "Department", dept_id)
        ...
        for _ in range(num_candidates):
            ...
            self.nodes["Candidate"].append({
                ...
                "source": random.choice(["LinkedIn", "Referral", "Indeed", "Company Website", "Agency"])
            })
            ...
            for job_id in applied_jobs:
                ...
                status = random.choice(["Rejected", "Withdrawn", "Hired", "In Progress"])
                
                self.nodes["Application"].append({
                    "id": app_id,
                    "applied_date": app_date.isoformat(),
                    "current_stage": random.choice(["Screen", "Technical", "Onsite", "Offer"]),
                    "status": status
                })
                
                self.add_rel("Candidate", cand_id, "SUBMITTED", "Application", app_id)
                self.add_rel("Application", app_id, "FOR_POSTING", "JobPosting", job_id)
                self.add_rel("Application", app_id, "MANAGED_BY", "Recruiter", random.choice(recruiters))
                ...
                for round_num in range(1, num_interviews + 1):
                    ...
                    int_date = app_date + timedelta(days=round_num * random.randint(3, 10))
                    self.nodes["Interview"].append({
                        "id": int_id,
                        "round_number": round_num,
                        "interview_type": random.choice(["Behavioral", "Technical", "Panel", "Hiring Manager"]),
                        "date": int_date.isoformat(),
                        "outcome": random.choice(["Pass", "Fail", "Borderline"])
                    })
                    self.add_rel("Application", app_id, "HAS_INTERVIEW", "Interview", int_id)
                    self.add_rel("Interview", int_id, "CONDUCTED_BY", "Interviewer", random.choice(dept_interviewers))
                ...
                if random.random() < 0.35:
                    ...
                    self.nodes["Review"].append({
                        "id": rev_id,
                        "text": generate_review_text(sentiment, "interview_experience"),
                ...
                if status == "Hired":
                    offer_id = f"OFF-{uuid.UUID(int=random.getrandbits(128), version=4)}"
                    ...
                    self.add_rel("Application", app_id, "RESULTED_IN", "Offer", offer_id)
                    ...
                    self.nodes["Employee"].append({
                        ...
                        "tenure_months": random.randint(1, 48) if not still_employed else random.randint(1, 60),
                    ...
                    if company_id and random.random() < 0.7:
                        ...
                        self.nodes["Review"].append({
                            "id": rev_id,
                            "text": generate_review_text(sentiment, "employee_experience"),
                            "rating": random.randint(4, 5) if sentiment == "positive" else (random.randint(1, 2) if sentiment == "negative" else 3),
                            "review_date": (hire_date + timedelta(days=random.randint(30, 365))).isoformat(),
                            "review_type": "employee_experience",
                            "recommends": True if sentiment == "positive" else False
                        })
