...
class PatternInjector:
    ...
    def inject_attrition_pattern(self):
        ...
        hired_cands = [e for e in self.nodes["Employee"] if e.get("converted_from_candidate")]
        ...
        for idx, emp in enumerate(attrition_group):
            ...
            if idx < 5:
                self.target_ids["attrition"].append({"cand_id": cand_id, "emp_id": emp["id"]})
            
            # 1. Negative Interview Review
            self._ensure_review(cand_id, "Candidate", "interview_experience", "negative")
            ...
            self._ensure_review(emp["id"], "Employee", "employee_experience", "negative", company_id=emp["company_id"])

        for idx, emp in enumerate(control_group):
            cand_id = emp["candidate_id"]
            if idx < 5:
                self.target_ids["control"].append({"cand_id": cand_id, "emp_id": emp["id"]})
            
            # 1. Positive Interview Review
            self._ensure_review(cand_id, "Candidate", "interview_experience", "positive")
            ...
            self._ensure_review(emp["id"], "Employee", "employee_experience", "positive", company_id=emp["company_id"])

    def _ensure_review(self, author_id, author_type, review_type, sentiment, company_id=None):
        # Find existing review by this author of this type
        existing_rel = next((r for r in self.relationships if r["source_id"] == author_id and r["source_type"] == author_type and r["rel_type"] == "WROTE"), None)
        ...
        if review_type == "interview_experience":
            ...
            app_rel = next((r for r in self.relationships if r["source_id"] == author_id and r["rel_type"] == "SUBMITTED"), None)
            if app_rel:
                job_rel = next((r for r in self.relationships if r["source_id"] == app_rel["target_id"] and r["rel_type"] == "FOR_POSTING"), None)
                if job_rel:
        ...
        if existing_rel:
            ...
            rev_node = next((n for n in self.nodes["Review"] if n["id"] == rev_id), None)
            if rev_node and rev_node["review_type"] == review_type:
                rev_node["text"] = generate_review_text(sentiment, review_type)
                rev_node["rating"] = random.randint(4, 5) if sentiment == "positive" else (random.randint(1, 2) if sentiment == "negative" else 3)
        ...
        self.nodes["Review"].append({
            ...
            "recommends": True if sentiment == "positive" else (False if sentiment == "negative" else None) if review_type == "employee_experience" else None
        })
    ...
    def inject_delayed_offer_pattern(self):
        # Pick one department
        if not self.nodes["Department"]: return
        target_dept = self.nodes["Department"][0]
        
        # Find applications for jobs in this dept that resulted in an offer
        dept_jobs = [r["target_id"] for r in self.relationships if r["source_id"] == target_dept["id"] and r["rel_type"] == "POSTED"]
        
        for app in self.nodes["Application"]:
            job_rel = next((r for r in self.relationships if r["source_id"] == app["id"] and r["rel_type"] == "FOR_POSTING"), None)
            offer_rel = next((r for r in self.relationships if r["source_id"] == app["id"] and r["rel_type"] == "RESULTED_IN"), None)
            
            if job_rel and job_rel["target_id"] in dept_jobs and offer_rel:
                # Inflate the gap between applied_date and offer decision_date
                offer_node = next(o for o in self.nodes["Offer"] if o["id"] == offer_rel["target_id"])
                ...
                offer_node["extended_date"] = (new_dec_date - timedelta(days=5)).isoformat()


    def inject_negative_interview_round_pattern(self):
        # Pick one JobPosting
        if not self.nodes["JobPosting"]: return
        target_job = self.nodes["JobPosting"][-1]
        
        # Find applications for this job
        apps_for_job = [r["source_id"] for r in self.relationships if r["target_id"] == target_job["id"] and r["rel_type"] == "FOR_POSTING"]
        
        for app_id in apps_for_job:
            # Find interviews for this application in round 3
            interview_rels = [r for r in self.relationships if r["source_id"] == app_id and r["rel_type"] == "HAS_INTERVIEW"]
            for ir in interview_rels:
                int_node = next(i for i in self.nodes["Interview"] if i["id"] == ir["target_id"])
                if int_node["round_number"] == 3:
                    # Inject extremely negative review specifically citing round 3
                    cand_id = next(r["source_id"] for r in self.relationships if r["target_id"] == app_id and r["rel_type"] == "SUBMITTED")
                    self._ensure_review(cand_id, "Candidate", "interview_experience", "negative")
                    
                    # Make it specifically about round 3
                    rev_rel = next((r for r in self.relationships if r["source_id"] == cand_id and r["rel_type"] == "WROTE"), None)
                    if rev_rel:
                        rev_node = next((n for n in self.nodes["Review"] if n["id"] == rev_rel["target_id"] and n["review_type"] == "interview_experience"), None)
                        if rev_node:
                            rev_node["text"] = f"Round 3 was a disaster. {rev_node['text']}"
