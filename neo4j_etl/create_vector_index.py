...
def backfill_embeddings(driver):
    ...
    with driver.session() as session:
        ...
        result = session.run("MATCH (r:Review) WHERE r.embedding IS NULL RETURN r.id AS id, r.text AS text")
        ...
        for i in range(0, total, batch_size):
            ...
            updates = [{"id": ids[j], "embedding": vectors[j]} for j in range(len(batch))]
