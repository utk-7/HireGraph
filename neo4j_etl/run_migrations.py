...
def run_constraints(driver):
    with open(os.path.join(os.path.dirname(__file__), "01_constraints.cypher"), "r") as f:
        content = f.read()
...
def load_data(driver, data_dir):
    ...
    migration_files = [f for f in files if f.endswith(".cypher") and f != "01_constraints.cypher"]
        ...
        with open(csv_path, "r", encoding="utf-8") as f:
            ...
            with driver.session() as session:
                for batch in batch_iterable(reader, 1000):
                    session.execute_write(lambda tx, q, p: tx.run(q, rows=p), cypher_query, batch)
